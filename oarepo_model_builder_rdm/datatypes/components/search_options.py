...
class RDMSearchOptionsModelComponent(DataTypeComponent):
    ...
    depends_on = [SearchOptionsModelComponent] #todo: depends on draft search options - when implemented
    ...
    def before_model_prepare(self, datatype, *, context, **kwargs):
        if datatype.root.profile == "record":
            ...
            datatype.definition["search-options"]["base-classes"] = ["invenio_rdm_records.services.config.RDMSearchOptions"]
        elif datatype.root.profile == "draft":
            datatype.definition["search-options"]["base-classes"] = ["invenio_rdm_records.services.config.RDMSearchDraftsOptions"]
